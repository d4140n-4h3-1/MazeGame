//! The head: how it is carried along with the body - bobbing in step with the stride, dipping
//! under a landing, rolling into movement and turns - and turned round to look behind. This is
...
/// that stopping or leaving the ground settles the head instead of stranding it mid-swing.
...
impl Player {
    ...
    /// how fast the body is travelling along the floor, and `right` is its right.
    pub(super) fn carry_head(&mut self, horizontal: Vector3<f32>, right: Vector3<f32>, dt: f32) {
        ...
        // Nothing swings while there is no floor to push off.
        let wanted = if self.grounded {
            (speed / self.posture.speed(Gait::Walking)).min(BOB_LIMIT)
        } else {
            0.0
        };
        ...
        let wanted = roll_degrees(
            ...
            self.posture.speed(Gait::Sprinting),
            yaw_rate,
        )
        .to_radians();
    ...
    /// Puts the camera where the head is: at the eyes' height, moved by the stride, the landing
    /// and the lean, and turned by the look behind, the pitch, the lean and the roll.
    pub(super) fn place_head(&self, graph: &mut Graph) {
        ...
        let head = graph[self.camera.transmute::<Node>()].local_transform_mut();
        head.set_position(Vector3::new(
            -self.lean - sideways,
            FEET + self.eyes - LEAN_DIP * leaning.abs() + up - self.landing,
            0.0,
        ));
        head.set_rotation(
            UnitQuaternion::from_axis_angle(&Vector3::y_axis(), self.look_back.angle())
...
mod tests {
    ...
    fn the_head_rises_and_falls_twice_a_stride_and_sways_once() {
        ...
        assert!(sideways(0.1) > 0.0 && sideways(0.6) < 0.0, "one foot, then the other");
    }
